                ...
                <v-row>
                    <v-col cols="5">
                        <div style="position: relative" class="mt-16">
                            <h1 class="text-grey">Ciao</h1>
                            <h1 class="text-white">Sono {{ utenteData.length > 0 ? utenteData[0].nome : 'Loading...' }}</h1>
                            <span class="text-grey">App Developer & Seo</span><br />
              ...
              " class="mt-16">
                    ...
                    <v-col cols="5">
                        <div style="position: relative; z-index: 9999" class="mt-16">
                         <v-img
        :src="utenteData.length > 0 ? utenteData[0].immagine : 'Nessuna immagine'"
        contain
        max-height="300"
        class="rounded-img"
      ></v-img>
...
//recupero dati dal portfolio.js per i dati in postgresql
...
export default {
    name: "HomeView",
     data() {
        return {
            utenteData: [], // Inizializza con un array vuoto
            jobData: [],    // Inizializza con un array vuoto
            slider2: 50,
            items: [
                {
                    img: "i3.jpg",
      ...
      mounted() {
        // Chiamate ai metodi del servizio per ottenere i dati e assegnarli alle variabili di stato
        this.fetchUtenteData();
        this.fetchJobData();
        // ... chiamate per gli altri tipi di dati
         if (this.utenteData.length > 0) {
            console.log("Valore dell'immagine:", this.utenteData[0].immagine);
        }
    },
    methods: {
        async fetchUtenteData() {
            try {
                const data = await PortfolioService.getUtenteData();
        ...
        async fetchJobData() {
            try {
                const data = await PortfolioService.getJobData();
                this.jobData = data;
            } catch (error) {
                console.error('Errore durante il recupero dei dati job:', error);
            }
        },
...
/* .rounded-img {
  border-radius: 8px; /* Regola il valore a tuo piacimento */
 /* position: relative;
} */
...
.image-overlay {
  ...
  width: 100%;
  height: 100%;
  background-color: rgba(0, 0, 0, 0.5); /* Imposta il colore e l'opacità desiderati */
  border-radius: 8px; /* Ripetiamo il valore del bordo arrotondato */
}
.hire {
    width: 100%;
    height: 200px;
